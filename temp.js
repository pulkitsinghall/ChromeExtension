...
//    }
// }
 function fillDataEvent(event , task){
     console.log("Inside fillDataEvent") ;
    if(prevXPath !== ""){
    let tarelement = getElementByXpath(prevXPath) ;
...
function eventHandlerInIframe(event){
    ...
    let XiPath = getXPath(document.activeElement) ;
...
Array.prototype.slice.call(allIframe).forEach(iframe => {
   ...
   // let idoc = iwindow.document ;
    iwindow.addEventListener('click' , eventHandlerInIframe);
        //  idoc.addEventListener('DOMSubtreeModified' , ()=>{
        //      findAllFrames(idoc) ;
        //  })
     }) ;
 }
...
//     let tarelement = event.target ;
//     let XPath = getXPath(tarelement) ;
//     console.log("This has been printed by mousedown event :" + XPath) ;
//     }
// })
